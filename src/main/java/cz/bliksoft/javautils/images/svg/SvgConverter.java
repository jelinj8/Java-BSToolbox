...
public class SvgConverter {
	...
	public static SVGDocument loadSvgDocument(String resourcePath) throws IOException {
		URL url = SvgConverter.class.getResource(resourcePath);
		if (url == null)
	...
	/**
	 * Renders a pre-loaded {@link SVGDocument} to a {@link BufferedImage}.
	 * <p>
	 ...
	 * </ul>
	 */
	public static BufferedImage createImageFromSVG(SVGDocument doc, Float width, Float height, Float scale) {
	...
	public static BufferedImage createImageFromSVG(File svg, Float width, Float height, Float scale) throws Exception {
		return createImageFromSVG(svg, width, height, scale, null, null);
	}

	public static BufferedImage createImageFromSVG(File svg, Float width, Float height, Float scale, String strokeColor,
			String fillColor) throws Exception {
		String svgContent;
		try (FileInputStream is = new FileInputStream(svg)) {
			svgContent = new String(IOUtils.toByteArray(is), StandardCharsets.UTF_8);
		}
	...
	public static BufferedImage createImageFromSVG(File svg, Float width, Float height) throws Exception {
		return createImageFromSVG(svg, width, height, null);
	}

	public static BufferedImage createImageFromSVG(File svg, Float height) throws Exception {
		return createImageFromSVG(svg, null, height, null);
	}

	public static BufferedImage createImageFromSVG(File svg) throws Exception {
		return createImageFromSVG(svg, null, null, null);
	}

	public static BufferedImage createImageFromSVGResource(String path, Float width, Float height, Float scale)
			throws Exception {
		return createImageFromSVGResource(path, width, height, scale, null, null);
	}

	public static BufferedImage createImageFromSVG(String path, Float width, Float height) throws Exception {
		return createImageFromSVGResource(path, width, height, null);
	}

	public static BufferedImage createImageFromSVG(String path, Float height) throws Exception {
		return createImageFromSVGResource(path, null, height, null);
	}

	public static BufferedImage createImageFromSVG(String path) throws Exception {
	...
	public static BufferedImage createImageFromSVGResource(String path, Float width, Float height, Float scale,
		...
		URL url = SvgConverter.class.getResource(path);
		if (url == null)
	...
	/**
	 * Loads an {@link SVGDocument} from in-memory SVG source text — used to
	 * rasterize synthetic, programmatically-built SVG documents (e.g. inline path
	 * data wrapped in a minimal {@code <svg>} envelope).
	 */
	public static SVGDocument loadSvgDocumentFromString(String svgContent) throws IOException {
		return loadSvgDocumentFromString(svgContent, null);
	}
