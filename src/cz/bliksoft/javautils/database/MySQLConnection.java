 ...
 */
public class MySQLConnection implements IDBConnectionFactory {
	...
	private void init() throws ClassNotFoundException, GeneralSecurityException, IOException {
		...
		if (driver == null) {
			...
			try {
				String driverName = "com.mysql.cj.jdbc.Driver";// $NON-NLS-1$ //$NON-NLS-1$
				...
				throw new ClassNotFoundException("Class com.mysql.cj.jdbc.Driver not found.", e);
			}
		}
	}
	...
	public Connection getConnection(String reason) throws Exception {
		if (reason == null) {
			...
			if ("MySQLConnection.java".equals(ste.getFileName())) {
