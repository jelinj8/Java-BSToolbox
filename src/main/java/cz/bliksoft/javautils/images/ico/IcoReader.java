 ...
 * {@link BufferedImage}. Supports PNG-in-ICO (Vista+), 32-bpp BGRA DIB, 24-bpp
...
public class IcoReader {
	...
	private static final class IcoEntry {
		...
		IcoEntry(int width, int height, int bitCount, int dataSize, int dataOffset) {
			this.width = width;
			this.height = height;
	...
	public static BufferedImage loadFromFile(File file, Integer targetW, Integer targetH) throws IOException {
		try (FileInputStream fis = new FileInputStream(file)) {
			return load(fis, targetW, targetH);
		}
	}

	public static BufferedImage loadFromResource(String resourcePath, Integer targetW, Integer targetH)
			throws IOException {
	...
	private static BufferedImage load(InputStream in, Integer targetW, Integer targetH) throws IOException {
	...
	private static IcoEntry buildEntry(byte[] b, int off) {
		...
		return new IcoEntry(w, h, bitCount, dataSize, dataOffset);
	}
	...
	private static IcoEntry selectBestEntry(List<IcoEntry> entries, Integer targetW, Integer targetH) {
		...
		if (target < 0) {
			return entries.stream().max(Comparator.comparingInt(e -> e.width() * e.height()))
